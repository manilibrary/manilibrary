...
export async function PATCH(request: Request) {
  ...
  if (tagline === undefined && taglineSub === undefined && galleryImageId === undefined) {
    return apiError("Provide tagline, taglineSub, and/or galleryImageId.", 400);
  }
  ...
  const { error } = await admin.from("library_settings").update(patch).eq("id", 1);
  if (error) {
    if (error.code === "23505") {
      return apiError("That gallery image is already used on another hero slot.", 400);
    }
