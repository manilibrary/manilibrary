...
import { SHORT_TERM_DURATION_OPTIONS, type ShortTermDurationKey } from "@/lib/payments/pricing";
...
export default function MembershipShortTermPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/memberships/seat-occupancy?planKind=short_term", { cache: "no-store" });
    ...
    <div className="mx-auto max-w-6xl px-5 py-10 md:px-8 md:py-14">
      ...
      <header className="max-w-2xl">
        ...
        <h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink-900 md:text-4xl">
          Choose your desk — 3-row hall
        </h1>
        <p className="mt-3 text-sm leading-relaxed text-ink-600">
          Same 90-seat row layout as the Expo short-term flow. Ideal for day or
          week passes; occupancy colours differ from long-term members.
        </p>
      </header>
      ...
      {!hasActive ? (
        <div className="mt-6 rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-xs text-ink-600">
            Start date is the calendar day your pass begins ({DEFAULT_LIBRARY_TZ}, from midnight). End time is set
          ...
          <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:flex-wrap">
            ...
            <label className="flex flex-col gap-1 text-xs text-ink-600">
              ...
              <select
                ...
                value={durationKey}
                onChange={(e) => setDurationKey(e.target.value as ShortTermDurationKey)}
              >
                {SHORT_TERM_DURATION_OPTIONS.map((o) => (
      ...
      <div className="mt-8 space-y-6">
        ...
        <MembershipSeatMapIntro mode="short" />
        <MembershipLegend mode="short" />
      </div>

      <div id="seat-map" className="mt-10 scroll-mt-24 space-y-4">
      ...
      <div className="sticky bottom-0 z-10 mt-10 border-t border-ink-100 bg-surface-muted/95 py-4 backdrop-blur supports-[backdrop-filter]:bg-surface-muted/80 md:static md:border-0 md:bg-transparent md:py-0">
        {hasActive ? (
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-ink-600">
              Pay securely with Razorpay (test mode) after choosing a seat. Slot / shift selection can follow later.
            </p>
